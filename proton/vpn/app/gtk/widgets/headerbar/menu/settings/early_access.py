...
class EarlyAccessWidget(ToggleWidget):
    ...
    def _find_installed_repo_packages(self) -> Tuple[bool, bool]:
        ...
        if self._command_failed(result):
            stderr = result.stderr.decode('utf-8') if result and hasattr(result, 'stderr') and result.stderr else "No result"
            logger.warning(
    ...
    def _run_commands(
        ...
        def on_handle_early_access(future: Future) -> None:
            ...
            if self._command_failed(result):
                stderr = result.stderr.decode('utf8') if result and hasattr(result, 'stderr') and result.stderr else ""
                stdout = result.stdout.decode('utf8') if result and hasattr(result, 'stdout') and result.stdout else ""
                logger.warning(
